(() => {
  ...
  function injectShell() {
    ...
    sub.insertAdjacentHTML("afterend", `
      <section class="panel" id="topVulnPanel">
        <div class="panel-head"><div class="panel-title">Top Pitcher Vulnerabilities</div><div class="panel-note" id="avgVuln">Loading</div></div>
  ...
  function renderTopVulnerabilities() {
    ...
    document.getElementById("avgVuln").textContent = avg.toFixed(1) + " avg pitcher vuln";
    document.getElementById("vulns").innerHTML = rows.map((row, index) => `
      <div class="vuln">
        <small>#${index + 1}</small>
        <b>${Math.round(row.score)}</b>
        <strong>${esc(row.pitcher)}</strong>
        <span>${esc(code(row.team))} pitcher vs ${esc(code(row.opponent))} bats</span>
      </div>
    `).join("");
  }
