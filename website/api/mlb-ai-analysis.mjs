...
export default async function handler(request, response) {
  ...
  try {
    ...
    const players = (Array.isArray(slate.players) ? slate.players : []).slice(0, 14).map(cleanPlayer).filter(player => player.playerId && player.player);
    ...
    const prompt = [
      ...
      "Select 4-6 genuinely distinct hitters. Explain signal agreement and disagreement in specific baseball language rather than repeating raw fields.",
    ...
    console.log("[mlb-ai-analysis] OpenAI request started", { playerCount: players.length, model: MODEL });
    ...
    console.error("[mlb-ai-analysis] request failed", { name: error?.name, message: error?.message });
    const message = error?.name === "TimeoutError" ? "OpenAI analysis took too long. Please try again." : error.message || "MLB AI analysis failed";
