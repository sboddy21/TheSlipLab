(function(){
  ...
  function renderBrief(){
    const scoped=enriched().filter(row=>!state.game||String(row.gameId)===state.game),brief=$("#wdcBrief");
    if(!scoped.length){brief.innerHTML=`<div class="wdc-change-empty">Today’s brief will populate when the active projection slate is available.</div>`;return}
    const strongest=[...scoped].sort((a,b)=>b.signal-a.signal)[0],minutes=[...scoped].sort((a,b)=>b.expectedMinutes-a.expectedMinutes)[0];
    const gameGroups=new Map();for(const row of scoped){const group=gameGroups.get(String(row.gameId))||[];group.push(row);gameGroups.set(String(row.gameId),group)}
    const paceSpot=[...gameGroups].map(([gameId,rows])=>({gameId,rows,pace:rows.reduce((sum,row)=>sum+Number(row.context?.paceFactor||1),0)/rows.length})).sort((a,b)=>b.pace-a.pace)[0];
    const paceGame=state.games.find(game=>String(game.gameId)===paceSpot.gameId),injuries=scoped.filter(row=>row.injury||row.playerData?.injury),scopeName=state.game?paceGame?.name||"Selected matchup":"Complete slate";
    $("#wdcBriefMeta").textContent=`${scopeName} · ${markets[state.market]} focus`;
  ...
  async function load(){
    try{
      const files=["wnba_live_snapshot.json","wnba_player_baselines.json","wnba_games_today.json","wnba_verified_markets.json","wnba_change_feed.json"];
  ...
  $("#wdcBrief").addEventListener("click",event=>{const player=event.target.closest("[data-brief-player]"),game=event.target.closest("[data-brief-game]");if(player)openPlayer(player.dataset.briefPlayer);else if(game)setGame(game.dataset.briefGame)});
