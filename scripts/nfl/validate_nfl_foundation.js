...
const odds = payloads["nfl_sportsbook_lines.json"];
...
if (odds.provider !== "The Odds API" || odds.freshnessPolicy?.staleQuotesAccepted !== false || odds.freshnessPolicy?.unmatchedQuotesAccepted !== false) fail("sportsbook contract is not fail-closed");
...
if (!tdBoard.rows?.length || tdBoard.rows.length !== tdBoard.counts?.rankedPlayers || tdBoard.counts?.publishableRecommendations !== 0 || tdBoard.counts?.verifiedSportsbookPrices !== tdBoard.rows.filter(row => row.gates?.verifiedSportsbookPrice).length) fail("TD Decision Center counts are invalid");
...
if (tdBoard.rows.some(row => row.gates?.verifiedSportsbookPrice !== row.gates?.freshMarketQuote || (row.gates?.verifiedSportsbookPrice && row.sportsbook?.status !== "fresh_verified") || (row.gates?.regularSeasonRoleConfirmed && !practice.officialReportsActive))) fail("TD Decision Center bypassed a launch gate");
const weatherByGame = new Map(weather.games.map(row => [row.gameId, row]));
if (tdBoard.rows.some(row => !row.opponent || !row.gameId || !row.gates?.verifiedOpponent || !row.gates?.defensiveMatchup || row.gates?.gameEnvironment !== (weatherByGame.get(row.gameId)?.weatherGate === true) || row.matchup?.status !== "verified_historical_baseline" || row.matchup?.marketImpliedTeamTotal !== null)) fail("TD Decision Center is missing verified matchup or weather context");
...
if (health.sources?.tdDecisionCenter?.status !== "private_shadow_only" || health.sources?.tdDecisionCenter?.publishableRecommendations !== 0 || health.sources?.tdDecisionCenter?.verifiedSportsbookPrices !== tdBoard.counts.verifiedSportsbookPrices) fail("health contract must keep the TD Decision Center private and gated");
...
if (!health.sources?.practiceReports || !health.sources?.weather || !health.sources?.sportsbookLines || health.sources?.receivingYards?.publishableRecommendations !== 0) fail("health contract is missing dress-rehearsal sources");
